...
TEST_FILE_BMD = os.path.join(TEST_DIR, "test_data", "pairedBrokenMissingData", "50d5f31a-a761-4c04-990c-e7668fe6bf00","data")
TEST_DATA_BMD = SingleLanePerSamplePairedEndFastqDirFmt(TEST_FILE_BMD, "r")
...
def test_view_artifcat_type():
    ...
    assert_raises(ValueError, exp2=_itsxpress._view_artifact_type(per_sample_sequence=TEST_DATA_BMD))
...
def test_fastq_id_maker():
    ...
    try:
        _itsxpress._fastq_id_maker(per_sample_sequences=test_data2,
                                   artifact_type=artifact_type)
        passed = True
    except (ValueError,
            NotADirectoryError,
    ...
    _itsxpress._fastq_id_maker(per_sample_sequences=test_data3,
                               artifact_type=artifact_type)
...
def test_set_fastqs_and_check():
    ...
    for sequence in sequence_set:
        exp1, exp2 = _itsxpress._set_fastqs_and_check(per_sample_sequences=test_data,
                                                      artifact_type=artifact_type,
                                                      sequence=sequence,
    ...
    test_data2 = SingleLanePerSamplePairedEndFastqDirFmt(test_file2, "r")
    sequences, single_end = _itsxpress._fastq_id_maker(per_sample_sequences=test_data2,
                                                       artifact_type=artifact_type)
    sequence_set = set(sequences)
    passed = False
    try:
        for sequence in sequence_set:
            _itsxpress._set_fastqs_and_check(per_sample_sequences=test_data2,
                                             artifact_type=artifact_type,
                                             sequence=sequence,
                                             single_end=single_end,
                                             threads=threads)
...
def test_main():
    ...
    region = "ITS1"
    try:
        _itsxpress.main(per_sample_sequences=test_data,
                        threads=threads,
                        taxa=taxa,
                        region=region)
    except ValueError:
