...
def test_view_artifcat_type():
    ...
    assert_raises(ValueError,
                  exp2=itsx._view_artifact_type(per_sample_sequence=test_data))
...
def test_fastq_id_maker():
    ...
    exp1, exp2 = itsx._fastq_id_maker(per_sample_sequences=test_data,
    ...
    test_data2 = SingleLanePerSamplePairedEndFastqDirFmt(test_file2, "r")
    try:
        itsx._fastq_id_maker(per_sample_sequences=test_data2,
                             artifact_type=artifact_type)
        passed = True
    except (ValueError,
            NotADirectoryError,
    ...
    itsx._fastq_id_maker(per_sample_sequences=test_data3,
                         artifact_type=artifact_type)
...
def test_set_fastqs_and_check():
    ...
    test_data = SingleLanePerSamplePairedEndFastqDirFmt(test_file, "r")
    artifact_type = "SampleData[PairedEndSequencesWithQuality]"
    sequences, single_end = itsx._fastq_id_maker(per_sample_sequences=test_data,
                                                 artifact_type=artifact_type)
    threads = 1
    ...
    for sequence in sequence_set:
        exp1, exp2 = itsx._set_fastqs_and_check(per_sample_sequences=test_data,
                                                artifact_type=artifact_type,
                                                sequence=sequence,
    ...
    test_data2 = SingleLanePerSamplePairedEndFastqDirFmt(test_file2, "r")
    sequences, single_end = itsx._fastq_id_maker(per_sample_sequences=test_data2,
                                                 artifact_type=artifact_type)
    sequence_set = set(sequences)
    passed = False
    try:
        for sequence in sequence_set:
            itsx._set_fastqs_and_check(per_sample_sequences=test_data2,
                                       artifact_type=artifact_type,
                                       sequence=sequence,
                                       single_end=single_end,
                                       threads=threads)
...
def test_main():
    ...
    region = "ITS1"
    try:
        itsx.main(per_sample_sequences=test_data,
                  threads=threads,
                  taxa=taxa,
                  region=region)
    except ValueError:
